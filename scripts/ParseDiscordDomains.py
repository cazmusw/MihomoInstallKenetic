...
async def main():
    ...
    os.makedirs(os.path.dirname(ALL_IP_LIST) or ".", exist_ok=True)

    async with aiofiles.open(ALL_IP_LIST, 'w') as f:
        pass
    ...
    with atqdm(total=total_domains, desc="🔍 DNS разрешение", unit=" domain") as pbar:
        tasks = [
    ...
    print()
    log_success(f'✅ Список обновлен: {YELLOW}{BOLD}{ALL_IP_LIST}{NC}')
    log_success(f"📊 Новых IP: {MAGENTA}{ip_count:,}{NC}")
    log_success(f"📊 Всего в истории: {MAGENTA}{len(merged_ips):,}{NC}")
    log_success(f"⏱️ Время выполнения: {MAGENTA}{elapsed:.1f}с{NC}")
    log_success(f"⚡ Скорость: {MAGENTA}{ip_count / elapsed:.0f} IP/с{NC}")
